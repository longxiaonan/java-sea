...
public class User {
    ...
    public User(Long uid, @NotBlank(message = "姓名不能为空") String username, @NotBlank(groups = Insert.class, message = "密码不能获取") String password,
                String sex, @Max(value = 200, message = "年龄不能超过 " + 200) @PositiveOrZero(message = "年龄必须大于等于0") Integer age, @Future(message = "时间必须是将来时间") String createTime, String remark, @NotBlank(message = "手机号不能为空") @Pattern(regexp = "^[1][3,4,5,6,7,8,9][0-9]{9}$", message = "手机号格式有误") String mobile, @NotBlank(message = "联系邮箱不能为空") @Email(message = "邮箱格式不对") String email) {
        this.uid = uid;
        this.username = username;
