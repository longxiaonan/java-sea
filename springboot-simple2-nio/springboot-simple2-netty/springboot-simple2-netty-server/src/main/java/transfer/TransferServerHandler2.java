...
//通过SimpleChannelInboundHandler<ByteBuf>来实现，通过channelRead0来读取数据，如果channelRead也在，那么先通过channelRead再通过channelRead0读取数据
@Sharable // 注解@Sharable可以让它在channels间共享
public class TransferServerHandler2 extends SimpleChannelInboundHandler<ByteBuf> {
	/**
	 * 当有收到数据的时候执行
	 * 
//	 * @param ctx
